...
export default class Pong extends Component {
  ...
  updateBall() {
    ...
    if (
      x <= 2 * paddleWidth &&
      y + 2 * R >= this.state.left.height &&
      y <= this.state.left.height + paddleHeight
    ) {
      dx = -dx; // Hit left paddle
    } else if (
      x + 2 * R >= W - 2 * paddleWidth &&
      y + 2 * R >= this.state.right.height &&
        ...
        <div style={{ textAlign: "center" }}>
          {this.state.winner !== false && (
            <h1>
              ...
              {this.state.winner === 0
                ? "Computer"
                : UserApi.getName(this.state.winner)}{" "}
              won
            </h1>
          )}
          <Button
      ...
      >
        ...
        <div
          style={{
            ...
            width: paddleWidth + "px",
            height: paddleHeight + "px",
            top: this.state.right.height + "px",
            right: paddleWidth + "px"
          }}
        />
        <div
          style={{
